...
#define MAX_FIDUCIAL_LIFE 3
#define MIN_FIDUCIAL_LIFE 2

#define FIDUCIAL_INVALID_COORDS -100.0f
...
public:
	...
	ofxFiducial() {
		fidId			= INVALID_ID;
		current.xpos	= current.ypos = FIDUCIAL_INVALID_COORDS;
		current.angle	= 0.0f;
	...
	void update(float _x, float _y, float _angle, float _root, float _leaf) {
		...
		if (life < MAX_FIDUCIAL_LIFE)
			life += 2; // 2 steps forward (isAlive() takes 1 step back)
//			++life;
	}
	...
	bool isAlive() {
		...
		if (!updated) {
			state			= FIDUCIAL_LOST;	//label as lost
