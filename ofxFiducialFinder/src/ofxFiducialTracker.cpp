...
ofxFiducialTracker::ofxFiducialTracker() {
	...
	cout << "Default data path: " << ofToDataPath("all.trees", true) << endl;
	initTree(ofToDataPath("all.trees", true).c_str());//put trees file in the data folder
...
//finds fiducials in image
void ofxFiducialTracker::findFiducials(ofImage& image)
{	
	findFiducials(image.getPixels(), image.width, image.height);
}

//finds fiducials in image
void ofxFiducialTracker::findFiducials(unsigned char* pixels, unsigned int w, unsigned int h)
{
	if (pixels==NULL || !w || !h)
		return;
	...
	for(int i = 0; i< fidCount; i++) {
		if(fiducials[i].id!=INVALID_FIDUCIAL_ID){
			...
			if (existing_fiducial!=NULL) {//if found
				//update fiducial
				existing_fiducial->update(fiducials[i].x,fiducials[i].y,
										  fiducials[i].angle + FIDUCIAL_ANGLE_CORRECTION_RADIANS,
										  fiducials[i].root_size,fiducials[i].leaf_size);
			} else {
