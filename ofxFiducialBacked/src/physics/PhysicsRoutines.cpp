...
namespace physics {
//--------------------------------------------------------------
Routines::Routines(fiducials_t& _fiducialsList)
: fiducialsList(_fiducialsList)
{
	functions_map[FIDUCIAL_PHYSICS_RAY]			= &Routines::findIntersectionsRay;
	functions_map[FIDUCIAL_PHYSICS_MULTI_RAY]	= &Routines::findIntersectionsMultiRay;
...
Routines::findMinIntersectionRay(ofxFiducial* fiducial, ofPoint _origin, double angle)
{
...
Routines::findIntersectionsRay(ofxFiducial* fiducial)
{
...
//--------------------------------------------------------------
queue<fiducialRayIntersectionEvtArgs>
Routines::findIntersectionsMultiRay(ofxFiducial* fiducial)
{
	queue<fiducialRayIntersectionEvtArgs> intersections;
	fiducialRayIntersectionEvtArgs args;
	...
	for (int output_idx=0; output_idx<rays.size(); output_idx++)
	{
		origin	= ofxPoint2f(fiducial->cornerPoints[0])
					.getMiddle(ofxPoint2f(fiducial->cornerPoints[3]));
		angle	= fiducial->getAngle()+FIDUCIAL_ANGLE_CORRECTION_RADIANS;
...
//--------------------------------------------------------------
queue<fiducialRayIntersectionEvtArgs>
Routines::findIntersectionsTri(ofxFiducial* fiducial)
{}

//--------------------------------------------------------------
queue<fiducialRayIntersectionEvtArgs>
Routines::findIntersectionsQuad(ofxFiducial* fiducial)
{}

//--------------------------------------------------------------
queue<fiducialRayIntersectionEvtArgs>
Routines::findIntersectionsPrism(ofxFiducial* fiducial)
{}

//--------------------------------------------------------------
vector<pair<ofPoint, double> >
Routines::getOutputRays(ofxFiducial* fiducial)
{
...
//--------------------------------------------------------------
vector<ofPoint>
Routines::getShape(ofxFiducial* fiducial)
{
...
//--------------------------------------------------------------
ofxPoint2f
Routines::intersects_window_edge(const ofxPoint2f& origin, const ofxPoint2f& endpoint)
{
...
ofxPoint2f
Routines::lineSegmentTest(const ofxPoint2f& origin, const ofxPoint2f& endpoint,
